...
---@field images_dir string directory of background images. Default is `wezterm.config_dir .. '/wallpapers/'`
...
--- Initialise wallpaper controller
---@private
function Wallpaper:init()
  local inital = {
    current_idx = 1,
    images = {},
    images_dir = wezterm.config_dir .. '/wallpapers/',
    focus_color = colors.background,
    focus_on = false,
  }
  local wallpapers = setmetatable(inital, self)
  return wallpapers
end

---Override the default `images_dir`
---Default `images_dir` is `wezterm.config_dir .. '/wallpapers/'`
---
--- INFO:
---  This function must be invoked before `set_images()`
---
---@param path string directory of background images
function Wallpaper:set_images_dir(path)
...
---MUST BE RUN BEFORE ALL OTHER `Wallpaper` functions
---Sets the `images` after instantiating `Wallpaper`.
---
--- INFO:
...
function Wallpaper:set_focus(focus_color)
...
---@private
---@return table
function Wallpaper:_create_opts()
  return {
    {
...
---@private
---@return table
function Wallpaper:_create_focus_opts()
  return {
    {
...
function Wallpaper:initial_options(focus_on)
  focus_on = focus_on or false
  assert(type(focus_on) == 'boolean', 'Wallpaper:initial_options - Expected a boolean')
...
function Wallpaper:_set_opt(window, background_opts)
...
---@private
---@param window any WezTerm Window see: https://wezfurlong.org/wezterm/config/lua/window/index.html
function Wallpaper:_set_focus_opt(window)
  local opts = {
    background = {
      {
        source = { Color = self.focus_color },
...
---@param window any? WezTerm `Window` see: https://wezfurlong.org/wezterm/config/lua/window/index.html
function Wallpaper:random(window)
  local new_idx
  repeat
    new_idx = math.random(#self.images)
  until new_idx ~= self.current_idx
...
---@param window any WezTerm `Window` see: https://wezfurlong.org/wezterm/config/lua/window/index.html
function Wallpaper:cycle_forward(window)
...
---@param window any WezTerm `Window` see: https://wezfurlong.org/wezterm/config/lua/window/index.html
function Wallpaper:cycle_back(window)
...
function Wallpaper:set_img(window, idx)
...
---@param window any WezTerm `Window` see: https://wezfurlong.org/wezterm/config/lua/window/index.html
function Wallpaper:toggle_focus(window)
